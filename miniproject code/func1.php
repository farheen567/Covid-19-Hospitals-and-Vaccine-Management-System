...
if(isset($_POST['docsub1'])){
	...
	$query = "select * from admins where username='$dname' and password='$dpass';";
	$result=mysqli_query($con,$query);
	$q2="select * from vadmin where username='$dname' and password='$dpass';";
	$r2=mysqli_query($con,$q2);
	if(mysqli_num_rows($result)==1)
	{
		$row1=mysqli_fetch_array($result,MYSQLI_ASSOC);
		$_SESSION['hpid']=$row1['hpid'];
		$_SESSION['dname']=$row1['username'];
		header("Location:doctor-panel.php");
	}
	else if(mysqli_num_rows($r2)==1)
	{
		$row2=mysqli_fetch_array($r2,MYSQLI_ASSOC);
		$_SESSION['dname']=$row2['username'];
		$_SESSION['vid']=$row2['vid'];
		header("Location:vadmin-panel.php");
